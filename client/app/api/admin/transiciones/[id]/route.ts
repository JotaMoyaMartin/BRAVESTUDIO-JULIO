...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await auth.admin.from('reel_transitions').update(update).eq('id', params.id).select().single()
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  const auth = await requireAdmin()
  if (!auth.ok) return NextResponse.json({ error: auth.msg }, { status: auth.status })
  const { error } = await auth.admin.from('reel_transitions').delete().eq('id', params.id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
...
export async function POST(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const auth = await requireAdmin()
  if (!auth.ok) return NextResponse.json({ error: auth.msg }, { status: auth.status })
  const { data: orig } = await auth.admin.from('reel_transitions').select('*').eq('id', params.id).single()
  if (!orig) return NextResponse.json({ error: 'No encontrada' }, { status: 404 })
  const { id: _id, created_at: _c, ...rest } = orig as ReelTransition
  const { data, error } = await auth.admin.from('reel_transitions').insert({
    ...rest,
    title: (orig as ReelTransition).title + ' (copia)',
    status: 'hidden',
  }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json({ item: data as ReelTransition })
