...
export default async function PlanificarPage({ searchParams }: { searchParams: Record<string, string> }) {
  ...
  if (!IS_CONFIGURED) {
    return <PlanificarClient userId="demo" brand={null} initialItems={[]} initialTab={initialTab as 'crear' | 'ver'} />
  }
  ...
  return (
    <PlanificarClient
      userId={user!.id}
      brand={brand as Pick<BrandProfile, 'optimized_summary' | 'salon_name' | 'main_services' | 'service_to_promote'> | null}
